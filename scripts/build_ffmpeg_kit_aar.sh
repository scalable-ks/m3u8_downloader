...
sed -i 's/ENABLED_LIBRARIES\[LIBRARY_CPU_FEATURES\]=1/ENABLED_LIBRARIES[LIBRARY_CPU_FEATURES]=0/' scripts/function-android.sh
cat > scripts/android/cpu-features.sh <<'EOF'
#!/bin/bash
exit 0
EOF
chmod +x scripts/android/cpu-features.sh
